...
const PDF_NAME_PART = (s) => String(s || '')
  .normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '')
  .replace(/[^a-z0-9]+/gi, '_')
  .replace(/^_+|_+$/g, '');

function generateInvoicePdf(invoice, lines, res, options = {}) {
  const entryDetailsByProject = (options.entryDetails || []).reduce((acc, entry) => {
  ...
  const alias = PDF_NAME_PART(invoice.company_invoice_alias);
  const number = PDF_NAME_PART(invoice.full_number || 'preview');
  const fileName = alias ? `Factura_${alias}_${number}.pdf` : `Factura_${number}.pdf`;
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader(
    'Content-Disposition',
    `${options.preview ? 'inline' : 'attachment'}; filename="${fileName}"`
  );
  doc.pipe(res);
  ...
  const drawPreviewWatermark = () => {
    ...
    preserveCursor(() => {
      ...
      doc.opacity(0.22)
         ...
         .fontSize(76)
         .font('Helvetica-Bold')
         .text('BORRADOR', -20, 340, { width: 640, align: 'center' })
         .fontSize(30)
         .text('NO VALIDO COMO FACTURA', -20, 425, { width: 640, align: 'center' });
      doc.opacity(1);
      doc.restore();
    });
  };
  ...
  const drawBrandFooter = () => {
    preserveCursor(() => {
      doc.save();
      doc.fillColor('#8a8aa8')
         .fontSize(7)
         .font('Helvetica')
         .text('Factura realizada con Cronoras · https://cronoras.up.railway.app/', 50, 780, {
           width: W,
           align: 'center',
           lineBreak: false
         });
  ...
  doc.on('pageAdded', drawBrandFooter);
  drawBrandFooter();
  ...
  doc.fillColor(GRAY).fontSize(8).font('Helvetica-Bold')
     .text('EMISOR', col1, y)
     .text('CLIENTE / DESTINATARIO', col2, y, { width: blockW, align: 'right' });
  y += 14;
  ...
  // Cliente
  doc.fillColor(BLACK).fontSize(10).font('Helvetica-Bold')
     .text(invoice.customer_name || '—', col2, y, { width: blockW, align: 'right' });
  doc.fillColor(GRAY).fontSize(9).font('Helvetica');
  if (invoice.customer_nif) doc.text(`NIF/CIF: ${invoice.customer_nif}`, col2, y + 14, { width: blockW, align: 'right' });
  const custLines = [
  ...
  custLines.forEach((l, i) => doc.text(l, col2, y + 26 + i * 13, { width: blockW, align: 'right' }));
  ...
  // ── LÍNEAS ───────────────────────────────────────────────────
  const colDesc = 50, colQty = 365, colPrice = 415, colTotal = 470;
  const descW = colQty - colDesc - 20;
  ...
  lines.forEach((line) => {
    const descHeight = doc.heightOfString(line.description, { width: descW, fontSize: 9 });
    const rowHeight = Math.max(descHeight, 14) + 4;
    if (line.project_id != null) {
      const bandY = y - 4;
      const lineY = y + Math.max(descHeight, 14) / 2 - 1;
      doc.rect(colDesc - 5, bandY, W + 10, rowHeight)
         .fill('#f7f7fb');
      doc.moveTo(colDesc + descW + 4, lineY)
         .lineTo(colQty - 8, lineY)
         .dash(1.5, { space: 2.5 })
         .strokeColor('#d2d5ea')
         .lineWidth(0.6)
         .stroke()
    ...
    doc.fillColor(BLACK).fontSize(9).font('Helvetica')
       .text(line.description, colDesc, y, { width: descW });
    ...
    const details = line.project_id != null ? entryDetailsByProject[String(line.project_id)] || [] : [];
    let previousDetailDate = null;
    details.forEach((entry) => {
      const detailDate = FECHA(entry.date);
      const visibleDate = detailDate && detailDate !== previousDetailDate ? detailDate : '';
      const detailDateW = 43;
      const detailGapW = 4;
      const detailText = `- ${entry.description || 'Sin descripcion'}`;
      previousDetailDate = detailDate || previousDetailDate;
      const detailDescX = colDesc + 10;
      const detailTextX = detailDescX + detailDateW + detailGapW;
      const detailTextW = colQty - detailTextX - 12;
      doc.fontSize(8).font('Helvetica');
      const detailHeight = doc.heightOfString(detailText, { width: detailTextW });
      doc.fillColor(GRAY).fontSize(8).font('Helvetica');
      if (visibleDate) doc.text(visibleDate, detailDescX, y, { width: detailDateW });
      doc.text(detailText, detailTextX, y, { width: detailTextW })
         .text(HORAS(entry.hours), colQty, y, { width: 55, align: 'right' });
      y += Math.max(detailHeight, 11) + 2;
      if (y > 680) {
        doc.addPage();
        y = 50;
      }
    });
    if (details.length) y += 4;
