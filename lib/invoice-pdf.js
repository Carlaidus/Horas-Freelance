...
function generateInvoicePdf(invoice, lines, res, options = {}) {
  ...
  lines.forEach((line) => {
    ...
    details.forEach((entry) => {
      ...
      const detailText = `${visibleDate ? visibleDate + '  -  ' : ''}${entry.description || 'Sin descripcion'}`;
      previousDetailDate = detailDate || previousDetailDate;
      const detailDescX = colDesc + 10;
      const detailDescW = colQty - detailDescX - 12;
      doc.fontSize(8).font('Helvetica');
      const detailHeight = doc.heightOfString(detailText, { width: detailDescW });
      doc.fillColor(GRAY).fontSize(8).font('Helvetica')
         .text(detailText, detailDescX, y, { width: detailDescW })
