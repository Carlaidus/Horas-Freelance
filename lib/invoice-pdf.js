...
function generateInvoicePdf(invoice, lines, res, options = {}) {
  ...
  res.setHeader(
    'Content-Disposition',
    `${options.preview ? 'inline' : 'attachment'}; filename="factura-${invoice.full_number || 'preview'}.pdf"`
  );
  doc.pipe(res);
  ...
  const drawPreviewWatermark = () => {
    ...
    preserveCursor(() => {
      ...
      doc.opacity(0.16)
         ...
         .fontSize(64)
         .font('Helvetica-Bold')
         .text('BORRADOR', 20, 350, { width: 560, align: 'center' })
         .fontSize(24)
         .text('NO VALIDO COMO FACTURA', 20, 420, { width: 560, align: 'center' });
      doc.opacity(1);
      doc.restore();
    });
  };
  ...
  doc.on('pageAdded', drawPreviewWatermark);
  ...
  // ── LÍNEAS ───────────────────────────────────────────────────
  const colDesc = 50, colQty = 355, colPrice = 415, colTotal = 470;
